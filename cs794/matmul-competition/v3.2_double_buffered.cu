...
// v3 + double buffering: keep the 64x64 output tile, 4x4 per-thread register
// tile, and float4 cooperative global loads, but split the K dimension into
// 32-wide shared-memory stages. Two 64x32 pages then ping-pong across the K
// loop so the next stage is filled before the current one is consumed.
constexpr int TILE = 64;
constexpr int R = 4;
constexpr int STAGE_K = 32;
constexpr int NUM_STAGES = 2;
constexpr int PAGE_FLOATS = TILE * STAGE_K;

__device__ inline void load_tile_page(const float *__restrict__ A,
  ...
  for (int idx = tid; idx < TILE * A_STAGE_VECS; idx += threads_per_block) {
    ...
    if (a_row < N && a_col + (VEC - 1) < N) {
      reinterpret_cast<float4 *>(&As[inner_row * STAGE_K + col4])[0] =
          reinterpret_cast<const float4 *>(&A[a_row * N + a_col])[0];
    } else {
#pragma unroll
      for (int v = 0; v < VEC; ++v) {
        const int g_col = a_col + v;
        As[inner_row * STAGE_K + col4 + v] =
  ...
  for (int idx = tid; idx < STAGE_K * B_TILE_VECS; idx += threads_per_block) {
    ...
    if (b_row < N && b_col + (VEC - 1) < N) {
      reinterpret_cast<float4 *>(&Bs[inner_row * TILE + col4])[0] =
          reinterpret_cast<const float4 *>(&B[b_row * N + b_col])[0];
    } else {
#pragma unroll
      for (int v = 0; v < VEC; ++v) {
        const int g_col = b_col + v;
        Bs[inner_row * TILE + col4 + v] =
...
__global__ void matmul_kernel_2d_block_tiled_double_buffered(
  ...
  float *As_pages = smem;
  float *Bs_pages = smem + NUM_STAGES * PAGE_FLOATS;
...
#pragma unroll
  ...
  load_tile_page(A, B, As_pages, Bs_pages, N, 0, tid, threads_per_block);
  __syncthreads();

  for (int t = 0; t < num_tiles; ++t) {
    const int stage = t & 1;
    const int next_stage = stage ^ 1;
    float *As = As_pages + stage * PAGE_FLOATS;
    float *Bs = Bs_pages + stage * PAGE_FLOATS;

    if (t + 1 < num_tiles) {
      float *As_next = As_pages + next_stage * PAGE_FLOATS;
      float *Bs_next = Bs_pages + next_stage * PAGE_FLOATS;
      load_tile_page(A, B, As_next, Bs_next, N, t + 1, tid, threads_per_block);
    }
...
#pragma unroll
      for (int i = 0; i < REG_TILE; ++i) {
        const float a_val = As[(local_row_base + i) * STAGE_K + k];
#pragma unroll
        for (int j = 0; j < REG_TILE; ++j) {
...
void matmul_gpu(int N, const float *A_h, const float *B_h, float *C_h) {
...

  {
    ...
    size_t shared_bytes = NUM_STAGES * 2 * PAGE_FLOATS * sizeof(float);

    matmul_kernel_2d_block_tiled_double_buffered<R>
