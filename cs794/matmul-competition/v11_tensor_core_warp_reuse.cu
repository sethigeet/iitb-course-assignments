...
// v11: each warp computes a 32x32 region (2x2 WMMA tiles), increasing reuse of
// both A and B fragments while the block covers a 64x128 output tile.
constexpr int WMMA_M = 16;
constexpr int WMMA_N = 16;
constexpr int WMMA_K = 8;
constexpr int WARP_SIZE = 32;
constexpr int WARP_GROUP_ROWS = 2;
constexpr int WARP_GROUP_COLS = 4;
constexpr int WARPS_PER_BLOCK = WARP_GROUP_ROWS * WARP_GROUP_COLS;
constexpr int THREADS_PER_BLOCK = WARPS_PER_BLOCK * WARP_SIZE;
constexpr int BLOCK_DIM_X = WARP_SIZE;
constexpr int BLOCK_DIM_Y = WARPS_PER_BLOCK;
constexpr int FRAGS_M_PER_WARP = 2;
constexpr int FRAGS_N_PER_WARP = 2;
constexpr int BLOCK_M = WARP_GROUP_ROWS * FRAGS_M_PER_WARP * WMMA_M;
constexpr int BLOCK_N = WARP_GROUP_COLS * FRAGS_N_PER_WARP * WMMA_N;
constexpr int BLOCK_K = 32;
constexpr int A_STRIDE = BLOCK_K + 1;
constexpr int B_STRIDE = BLOCK_N + 8;

static_assert(BLOCK_M == 64 && BLOCK_N == 128,
              "v11 expects a 64x128 block tile.");
static_assert(THREADS_PER_BLOCK == 256, "v11 expects 256 threads per block.");
...
__device__ inline void load_a_stage(const float *__restrict__ A, float *As,
  ...
  for (int idx = tid; idx < total_vecs; idx += THREADS_PER_BLOCK) {
    ...
    if (g_row < N) {
      const float4 a_vec =
          reinterpret_cast<const float4 *>(&A[g_row * N + k_base + col4])[0];
      As[row * A_STRIDE + col4 + 0] = a_vec.x;
      As[row * A_STRIDE + col4 + 1] = a_vec.y;
      As[row * A_STRIDE + col4 + 2] = a_vec.z;
      As[row * A_STRIDE + col4 + 3] = a_vec.w;
    } else {
      As[row * A_STRIDE + col4 + 0] = 0.0f;
      As[row * A_STRIDE + col4 + 1] = 0.0f;
      As[row * A_STRIDE + col4 + 2] = 0.0f;
      As[row * A_STRIDE + col4 + 3] = 0.0f;
    }
  }
}
...
__device__ inline void load_b_stage(const float *__restrict__ B, float *Bs,
  ...
  for (int idx = tid; idx < total_vecs; idx += THREADS_PER_BLOCK) {
    const int row = idx / VECS_PER_ROW;
    const int col4 = (idx % VECS_PER_ROW) * VEC;
    const int g_col = col_base + col4;
    if (g_col + (VEC - 1) < N) {
      reinterpret_cast<float4 *>(&Bs[row * B_STRIDE + col4])[0] =
          reinterpret_cast<const float4 *>(&B[(k_base + row) * N + g_col])[0];
    } else {
      Bs[row * B_STRIDE + col4 + 0] =
          (g_col + 0 < N) ? B[(k_base + row) * N + g_col + 0] : 0.0f;
      Bs[row * B_STRIDE + col4 + 1] =
          (g_col + 1 < N) ? B[(k_base + row) * N + g_col + 1] : 0.0f;
      Bs[row * B_STRIDE + col4 + 2] =
          (g_col + 2 < N) ? B[(k_base + row) * N + g_col + 2] : 0.0f;
      Bs[row * B_STRIDE + col4 + 3] =
          (g_col + 3 < N) ? B[(k_base + row) * N + g_col + 3] : 0.0f;
    }
  }
}

__global__ void
matmul_kernel_tensor_core_warp_reuse(const float *__restrict__ A,
  ...
  const int warp_row_group = warp_id / WARP_GROUP_COLS;
  const int warp_col_group = warp_id % WARP_GROUP_COLS;
  const int row_base = blockIdx.y * BLOCK_M;
  const int col_base = blockIdx.x * BLOCK_N;

  wmma::fragment<wmma::accumulator, WMMA_M, WMMA_N, WMMA_K, float>
      c_frag[FRAGS_M_PER_WARP][FRAGS_N_PER_WARP];
#pragma unroll
  for (int i = 0; i < FRAGS_M_PER_WARP; ++i) {
#pragma unroll
...
#pragma unroll
      for (int i = 0; i < FRAGS_M_PER_WARP; ++i) {
        const int row_offset = (warp_row_group * FRAGS_M_PER_WARP + i) * WMMA_M;
        wmma::load_matrix_sync(a_frag[i], &As[row_offset * A_STRIDE + kk],
                               A_STRIDE);
      }
...
#pragma unroll
      for (int j = 0; j < FRAGS_N_PER_WARP; ++j) {
        ...
        wmma::load_matrix_sync(b_frag[j], &Bs[kk * B_STRIDE + col_offset],
                               B_STRIDE);
      }
...
#pragma unroll
    for (int j = 0; j < FRAGS_N_PER_WARP; ++j) {
      const int c_row =
          row_base + (warp_row_group * FRAGS_M_PER_WARP + i) * WMMA_M;
      const int c_col =
          col_base + (warp_col_group * FRAGS_N_PER_WARP + j) * WMMA_N;
      if (c_row < N && c_col < N) {
        wmma::store_matrix_sync(C + c_row * N + c_col, c_frag[i][j], N,
                                wmma::mem_row_major);
      }
    }
  }
...
void matmul_gpu(int N, const float *A_h, const float *B_h, float *C_h) {
...

  {
    ...
    size_t shared_bytes =
        (BLOCK_M * A_STRIDE + BLOCK_K * B_STRIDE) * sizeof(float);

    matmul_kernel_tensor_core_warp_reuse<<<grid, block, shared_bytes>>>(
        A_d, B_d, C_d, N);
