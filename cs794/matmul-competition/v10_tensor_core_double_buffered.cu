...
// v10: ping-pong between two 32-wide K stages so one outer-loop iteration
// covers 64 columns of K, and software-pipeline fragment loads within each page.
...
constexpr int BLOCK_K = STAGE_K * NUM_STAGES;
constexpr int A_STRIDE = STAGE_K + 1;
constexpr int B_STRIDE = BLOCK_N + 8;
constexpr int A_PAGE_FLOATS = BLOCK_M * A_STRIDE;
constexpr int B_PAGE_FLOATS = STAGE_K * B_STRIDE;

static_assert(BLOCK_M == 64 && BLOCK_N == 64, "v10 is tuned for a 64x64 tile.");
static_assert(THREADS_PER_BLOCK == 256, "v10 expects 256 threads per block.");

__device__ inline void load_a_page(const float *__restrict__ A, float *As_page,
                                   int row_base, int k_base, int N, int tid) {
  constexpr int VEC = 4;
  constexpr int VECS_PER_ROW = STAGE_K / VEC;
  ...
  for (int idx = tid; idx < total_vecs; idx += THREADS_PER_BLOCK) {
    ...
    As_page[row * A_STRIDE + col4 + 0] = a_vec.x;
    As_page[row * A_STRIDE + col4 + 1] = a_vec.y;
    As_page[row * A_STRIDE + col4 + 2] = a_vec.z;
    As_page[row * A_STRIDE + col4 + 3] = a_vec.w;
  }
}

__device__ inline void load_b_page(const float *__restrict__ B, float *Bs_page,
                                   int col_base, int k_base, int N, int tid) {
  constexpr int VEC = 4;
  constexpr int VECS_PER_ROW = BLOCK_N / VEC;
  const int total_vecs = STAGE_K * VECS_PER_ROW;
  for (int idx = tid; idx < total_vecs; idx += THREADS_PER_BLOCK) {
    const int row = idx / VECS_PER_ROW;
    const int col4 = (idx % VECS_PER_ROW) * VEC;
    reinterpret_cast<float4 *>(&Bs_page[row * B_STRIDE + col4])[0] =
        reinterpret_cast<const float4 *>(&B[(k_base + row) * N + col_base + col4])[0];
  }
}

__global__ void matmul_kernel_tensor_core_double_buffered(
    const float *__restrict__ A, const float *__restrict__ B,
    float *__restrict__ C, int N) {
  extern __shared__ float smem[];
  float *As = smem;
  float *Bs = smem + NUM_STAGES * A_PAGE_FLOATS;
...
#pragma unroll
  ...
  for (int k_block = 0; k_block < N; k_block += BLOCK_K) {
#pragma unroll
    for (int stage = 0; stage < NUM_STAGES; ++stage) {
      float *As_page = As + stage * A_PAGE_FLOATS;
      float *Bs_page = Bs + stage * B_PAGE_FLOATS;
      const int stage_k_base = k_block + stage * STAGE_K;
      load_a_page(A, As_page, row_base, stage_k_base, N, tid);
      load_b_page(B, Bs_page, col_base, stage_k_base, N, tid);
    }
    __syncthreads();

#pragma unroll
    for (int stage = 0; stage < NUM_STAGES; ++stage) {
      const float *As_page = As + stage * A_PAGE_FLOATS;
      const float *Bs_page = Bs + stage * B_PAGE_FLOATS;
      ...
      wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_N, WMMA_K,
                     wmma::precision::tf32, wmma::row_major>
          b_frag[FRAGS_N_PER_WARP];
      wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K,
                     wmma::precision::tf32, wmma::row_major>
          a_next;
      wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_N, WMMA_K,
                     wmma::precision::tf32, wmma::row_major>
          b_next[FRAGS_N_PER_WARP];

      wmma::load_matrix_sync(a_frag, &As_page[(warp_row * WMMA_M) * A_STRIDE], A_STRIDE);
#pragma unroll
      for (int j = 0; j < FRAGS_N_PER_WARP; ++j) {
        const int col_offset = (warp_col_group * FRAGS_N_PER_WARP + j) * WMMA_N;
        wmma::load_matrix_sync(b_frag[j], &Bs_page[col_offset], B_STRIDE);
      }

#pragma unroll
      for (int kk = 0; kk < STAGE_K; kk += WMMA_K) {
        if (kk + WMMA_K < STAGE_K) {
          const int next_kk = kk + WMMA_K;
          wmma::load_matrix_sync(
              a_next, &As_page[(warp_row * WMMA_M) * A_STRIDE + next_kk], A_STRIDE);
#pragma unroll
          for (int j = 0; j < FRAGS_N_PER_WARP; ++j) {
            const int col_offset = (warp_col_group * FRAGS_N_PER_WARP + j) * WMMA_N;
            wmma::load_matrix_sync(b_next[j], &Bs_page[next_kk * B_STRIDE + col_offset],
                                   B_STRIDE);
          }
        }
...
void matmul_gpu(int N, const float *A_h, const float *B_h, float *C_h) {
...

  {
    ...
    matmul_kernel_tensor_core_double_buffered<<<grid, block, shared_bytes>>>(
        A_d, B_d, C_d, N);
