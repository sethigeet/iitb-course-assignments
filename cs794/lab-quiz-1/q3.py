...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, kv_cache=None):
        ...
        # TODO: with prefix sharing, you will have to adjust this mask
        if kv_cache is not None:
            mask = torch.ones(att.size(-2), att.size(-1))
            mask[:, -T:] = torch.tril(torch.ones(T, T))
            mask = mask.view(1, 1, att.size(-2), att.size(-1))
...
class Block(nn.Module):
    ...
    def forward(self, x, kv_cache):
        x_attn, kv = self.attn(self.ln_1(x), kv_cache)
        x = x + x_attn
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, kv_cache=None, prefix_len=0):
        device = idx.device
        b, t = idx.size()
        assert t <= self.config.block_size, f"Cannot forward sequence of length {t}, block size is only {self.config.block_size}"
        # TODO: with prefix sharing, you will have to adjust these positional indices
        pos = torch.arange(prefix_len, prefix_len+t, dtype=torch.long, device=device) # shape (t)
    ...
    @torch.no_grad()
    def generate(self, idx, max_new_tokens, enable_prefix_sharing=False):
        """
            Parameters
            ----------
            ...
            shared_prefix_kvs : List[Tuple[torch.Tensor, torch.Tensor]] or None
                ...
                - No common prefix exists (prefix_len == 0)
        """  
        device = next(self.parameters()).device

        sequences = [seq.to(device) if seq.device != device else seq for seq in idx]
        ...
        if enable_prefix_sharing:
            longest_prefix = []
            for i in range(1, min(len(s[0]) for s in sequences)):
                valid = True
                prefix = sequences[0][0][:i]
                for s in sequences:
                    if not torch.all(s[0][:i] == prefix):
                        valid = False
                        break
            ...
            if len(longest_prefix) > 0:
                _, _, kv_cache = self(longest_prefix.view(1, -1))
        ...
        for idx in sequences:
            for _ in range(max_new_tokens):
                ...
                if enable_prefix_sharing:
                    idx_cond = idx_cond[:, len(longest_prefix):]
                logits, _, _ = self(idx_cond, kv_cache=kv_cache, prefix_len=prefix_len)
