...
int main() {
    ...
    for (int i = 0; i < S; i++) cudaStreamCreate(&streams[i]);
    
    dim3 blockSize(k, k);
    dim3 gridSize((N+k-1)/k, ((B/S)+k-1)/k);
    for (int i = 0; i < S; i++) {
        int delta = i * N * (B/S);
        cudaMemcpyAsync(dX+delta, hX+delta, (B/S)*N*sizeof(float), cudaMemcpyHostToDevice, streams[i]);
        matmul<<<gridSize, blockSize, 2*k*k*sizeof(float), streams[i]>>>(dX+delta, dW1, dZ+delta, N, B/S, k);
        relu<<<1, N*B/S, 0, streams[i]>>>(dZ+delta, N, B/S);
        matmul<<<gridSize, blockSize, 2*k*k*sizeof(float), streams[i]>>>(dZ+delta, dW2, dX+delta, N, B/S, k);
    }

    cudaDeviceSynchronize();
    
    for (int i = 0; i < S; i++) cudaStreamDestroy(streams[i]);
