...
void getsiblings()
{
  ...
  struct proc *p;
  cprintf("Siblings of %d are: ", curproc->pid);
  int numsiblings = 0;
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
    if (p->parent->pid == curproc->parent->pid && p->pid != curproc->pid) {
      cprintf("%d ", p->pid);
      numsiblings++;
    }
  }
  cprintf("\nNumber of siblings are: %d\n", numsiblings);
