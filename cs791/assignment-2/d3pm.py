...
from utils import discretize_mnist, seed_everything
...
def train(
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_idx, (data, _) in enumerate(pbar):
            ...
            logits = model(noisy_data.float(), timesteps)
...
def sample(model, device, num_samples=16, num_steps=1000):
    """
    Sample from the D3PM model using the reverse diffusion process.

    Args:
        model: Trained D3PM model
...
if __name__ == "__main__":
    ...
    run_name = f"exps_d3pm/{args.epochs}ep_{args.batch_size}bs_{args.learning_rate}lr_{args.num_steps}steps_{args.scheduler_type}scheduler"
