...
def sample(
    model, class_label, device, num_samples=16, num_steps=1000, scheduler_type="cosine"
):
    """
    Returns:
    ...
    with torch.no_grad():
        ...
        class_labels = torch.full(
            (num_samples,), int(class_label), device=device, dtype=torch.long
        )
        for t in tqdm(reversed(range(num_steps)), desc=f"Sampling class {class_label}"):
            t_batch = torch.full((num_samples,), t, device=device, dtype=torch.long)
            eps_pred = model(x_t, t_batch, class_labels)
...
if __name__ == "__main__":
    ...
    run_name = f"exps_conditional_ddpm/{args.epochs}ep_{args.batch_size}bs_{args.learning_rate}lr_{args.num_steps}steps_{args.scheduler_type}scheduler"  # Change run name based on your experiments
