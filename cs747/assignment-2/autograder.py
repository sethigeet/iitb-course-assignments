...
class VerifyOutputPlanner:
    def __init__(self, algorithm, print_error):
        ...
        for in_file in policy_eval_files:
            cmd_planner = (
                ...
                in_file,
                "--policy",
                in_file.replace("continuing", "policy-continuing").replace(
    ...
    def verifyOutput(self, cmd_output, in_file, pe, pol_eval=False):
        sol_file = in_file.replace("continuing", "sol-continuing").replace(
            "episodic", "sol-episodic"
        )
        if pol_eval:
            sol_file = in_file.replace("continuing", "sol-policy-continuing").replace(
                "episodic", "sol-policy-episodic"
            )
        ...
        if not len(est) == nstates:
            ...
            print(
                "\n",
                "*" * 10,
                "Mistake:Exact number of line in standard output should be",
                nstates,
                "but have",
                len(est),
                "*" * 10,
        ...
        for i in range(len(est)):
            if not len(est[i]) == 2:
                ...
                print(
                    "\n",
                    "*" * 10,
                    "Mistake: On each line you should print only value,policy for a state",
                    "*" * 10,
                )
                break
...
if __name__ == "__main__":
    ...
    if args.task == 1:
        ...
        in_file_sol_ls = [
            "data/test/test_" + str(i) + "_solution.txt" for i in range(0, 5)
        ]

        i = 0
        for in_file, in_file_test, in_file_sol in zip(
            in_file_ls, in_file_test_ls, in_file_sol_ls
        ):
