...
def run_command(command, capture_stdout=False):
    ...
    try:
        ...
        print(
            f"❌ Error: Command not found. Is '{command.split()[0]}' installed and in your PATH?"
        )
        exit(1)
...
def main():
    parser = argparse.ArgumentParser(
        description="Master script for the MDP card game pipeline."
    )
    parser.add_argument(
        "--limit",
        type=int,
        required=True,
        help="A positive integer for the maximum sum.",
    )
    parser.add_argument(
        "--bonus",
        type=int,
        required=True,
        help="A positive integer for the bonus score.",
    )
    parser.add_argument(
        "--sequence",
        type=int,
        nargs=3,
        required=True,
        help="Three consecutive positive integers.",
    )
    parser.add_argument(
        "--initial_state",
        nargs="+",
    ...
    if args.bonus <= 0:
        parser.error("--bonus must be a positive integer.")
    seq = sorted(args.sequence)
    if not (seq[1] == seq[0] + 1 and seq[2] == seq[1] + 1):
        parser.error(
            f"--sequence values must be consecutive. You provided: {args.sequence}"
        )
    if not all(n > 0 and n < 13 for n in seq):
        parser.error(
            f"--sequence values must be positive and < 13. You provided: {args.sequence}"
        )
    ...
    try:
        ...
        run_command(
            f"python3 encoder.py --game_config {ENCODER_INPUT_FILE} > {MDP_FILE}"
        )
        print(f"✅ Encoder created '{MDP_FILE}'.")

        # --- Step 2: Planning with value iteration ---
        print("\n--- Step 2: Planning ---")
        planner_output = run_command(
            f"python3 planner.py --mdp {MDP_FILE}", capture_stdout=True
        )
        ...
        print("✅ Decoder generated the complete policy.")
