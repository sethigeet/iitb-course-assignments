...
def calculate_all_moves(dims):
    ...
    all_moves_inv = np.zeros(
        (total + (max(promotions) + 1) * len(promotions_dy), 3), dtype=np.int8
    )
    # index: [dx, dy, prom]
...
def flat_move_to_partial(all_moves_inv, dims, flat_move_index, color: bool):
    i, j, ind = np.unravel_index(
        flat_move_index, (dims[0], dims[1], all_moves_inv.shape[0])
    )
