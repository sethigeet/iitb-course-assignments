...
class Chess:
    ...
    def fen(self):
        ...
        en_passant_square = (
            "-"
            if not self.has_en_passant
            else files[self.en_passant[1]]
            + str((self.dims[0] + 1) - int(ranks[self.en_passant[0]]))
        )
        ...
        fen_string += " {} {} {} {} {}".format(
            ...
            castling_rights,
            en_passant_square,
            self.ply_count_without_adv,
            self.half_move_count // 2 + 1,
        )
        return fen_string
    ...
    def diagonal_move_magic(self, all_pieces: np.uint64, i: np.uint8, j: np.uint8):
        ...
        return (
            self.move_magic(
                all_pieces & self.DIAGONAL_MOVES[i, j],
                i,
                j,
                self.diagonal_magics,
                self.diagonal_hash_table,
                self.diagonal_magic_shift,
            )
            & self.DIAGONAL_MOVES[i, j]
        )
    ...
    def straight_move_magic(self, all_pieces: np.uint64, i: np.uint8, j: np.uint8):
        ...
        return (
            self.move_magic(
                all_pieces & self.STRAIGHT_MOVES[i, j],
                i,
                j,
                self.straight_magics,
                self.straight_hash_table,
                self.straight_magic_shift,
            )
            & self.STRAIGHT_MOVES[i, j]
        )
    ...
    def move_pieces(
        ...
        self.bitboards[self.turn, piece_at] = unset_bit(
            self.bitboards[self.turn, piece_at], f_from
        )
        self.piece_lookup[self.turn, origin[0], origin[1]] = -1
        piece_at = piece_at if promotion == -1 else promotion
        self.bitboards[self.turn, piece_at] = set_bit(
            self.bitboards[self.turn, piece_at], f_to
        )
        self.piece_lookup[self.turn, target[0], target[1]] = piece_at

    def make_move(
        self, i: np.uint8, j: np.uint8, dx: np.int8, dy: np.int8, promotion=-1
    ):
        """
        ...
        if piece_to == 3:
            ...
            if (i + dx) == back_rank and (
                (j + dy) == 0 or (j + dy) == self.dims[1] - 1
            ):
        ...
        if (
            piece_at == 0
            and self.has_en_passant
            and self.en_passant[0] == i + dx
            and self.en_passant[1] == j + dy
        ):
        ...
        for piece_type in range(5):
            self.bitboards[enemy_turn, piece_type] = unset_bit(
                self.bitboards[enemy_turn, piece_type], f_to
            )
    ...
    def insufficient_material(self):
        ...
        for turn in [0, 1]:
            ...
            if (
                self.bitboards[turn, 0]
                | self.bitboards[turn, 3]
                | self.bitboards[turn, 4]
            ) != 0:
                return False
            total = 0
    ...
    def can_castle(
        self, side: bool, all_pieces: np.uint64, king_danger_squares: np.uint64
    ):
        ...
        if not has_bit(self.bitboards[self.turn, 3], flat(back_rank, j, self.dims)):
            return False
        return (
            self.castling_rights[self.turn, side]
            and (self.CASTLING_ATTACK_MASKS[self.turn, side] & king_danger_squares) == 0
    ...
    def find_checkers(
        ...
        checkers |= self.diagonal_move_magic(all_pieces, i, j) & (
            self.bitboards[enemy_turn, 2] | self.bitboards[enemy_turn, 4]
        )
        # Same, but with pieces attacking in straight lines
        checkers |= self.straight_move_magic(all_pieces, i, j) & (
            self.bitboards[enemy_turn, 3] | self.bitboards[enemy_turn, 4]
        )
    ...
    def find_pinned_ray(
        ...
        if straight:
            result = (
                self.straight_move_magic(
                    all_pieces_without_pinned, king_pos[0], king_pos[1]
                )
                | king_mask
            ) & (
                self.straight_move_magic(
                    all_pieces_without_pinned, attacker[0], attacker[1]
                )
                | attacker_mask
            )
        else:
            result = (
                self.diagonal_move_magic(
                    all_pieces_without_pinned, king_pos[0], king_pos[1]
                )
                | king_mask
            ) & (
    ...
    def find_pinned_pieces(
        ...
        squares_to_check = (
            self.DIAGONAL_MOVES[king_pos[0], king_pos[1]]
            | self.STRAIGHT_MOVES[king_pos[0], king_pos[1]]
            | self.single_to_bitboard(king_pos[0], king_pos[1])
        ) & enemy_pieces
        for bit in true_bits(squares_to_check):
            i, j = unflat(bit, self.dims)
            piece_at = self.piece_at(i, j, enemy_turn)
            found = False
            if self.is_on_straight((i, j), king_pos) and (
                piece_at == 3 or piece_at == 4
            ):
                straight_pinned = (
                    self.straight_move_magic(all_pieces, i, j) & king_straights
                )
                if straight_pinned:
                    ...
                    pin_rays[pinned_piece[0], pinned_piece[1]] = self.find_pinned_ray(
                        all_pieces, king_pos, (i, j), pinned_piece, True
                    )
                    found = True
            if (
                self.is_on_diagonal((i, j), king_pos)
                and not found
                and (piece_at == 2 or piece_at == 4)
            ):
                diagonal_pinned = (
                    self.diagonal_move_magic(all_pieces, i, j) & king_diagonals
                )
                if diagonal_pinned:
                    # There is a pinned piece here!
                    # There can maximally be one piece per ray.
                    ...
                    pin_rays[pinned_piece[0], pinned_piece[1]] = self.find_pinned_ray(
                        all_pieces, king_pos, (i, j), pinned_piece, False
                    )

        return pin_rays

    def find_and_validate_en_passant_moves(
        self,
        all_pieces: np.uint64,
        opp_pawn_row_inc: np.int8,
        enemy_turn: bool,
        king_pos: Tuple[int, int],
    ):
        ...
        if self.has_en_passant:
            ...
            for dx in [-1, 1]:
                to_remove = self.single_to_bitboard(
                    i_act + opp_pawn_row_inc, j_act
                ) | self.single_to_bitboard(i_act + opp_pawn_row_inc, j_act + dx)
                # If there is an intersection between the king and some
                intersections = self.straight_move_magic(
                    all_pieces & ~to_remove, king_pos[0], king_pos[1]
                )
                # Find all sliding rays from opponent
                ...
                if (
                    has_bit(
                        intersections & opponent_straights,
                        flat(king_pos[0], king_pos[1], self.dims),
                    )
                    == 0
                ):
    ...
    def legal_moves(self):
        ...
        if checkers != 0 and not more_than_one_checker:
            ...
            if piece_at == 0 and self.has_en_passant:
                en_passant_capture_mask |= self.single_to_bitboard(
                    self.en_passant[0], self.en_passant[1]
                )
            ...
            if piece_at == 0 or piece_at == 1:
                ...
                intersections = B_0
                if piece_at == 2:
                    intersections |= self.diagonal_move_magic(
                        all_pieces, i, j
                    ) & self.diagonal_move_magic(all_pieces, king_pos[0], king_pos[1])
                if piece_at == 3:
                    intersections |= self.straight_move_magic(
                        all_pieces, i, j
                    ) & self.straight_move_magic(all_pieces, king_pos[0], king_pos[1])
                ...
                if piece_at == 4:
                    if king_pos[0] == i or king_pos[1] == j:
                        intersections |= self.straight_move_magic(
                            ...
                            all_pieces, king_pos[0], king_pos[1]
                        )
                    else:
                        intersections |= self.diagonal_move_magic(
                            all_pieces, i, j
        ...
        # Pinned pieces
        pin_masks = self.find_pinned_pieces(
            all_pieces, enemy_pieces, enemy_turn, king_pos
        )
        # Sweet! Now any piece that falls in this pinned_check_ray has to move only within that.

        en_passant_moves = self.find_and_validate_en_passant_moves(
            all_pieces, opp_pawn_row_inc, enemy_turn, king_pos
        )
        ...
        for bit in true_bits(my_pieces):
            ...
            if piece_at == 0:
                ...
                moves_to_make |= self.PAWN_ATTACKS[self.turn, i, j] & (
                    enemy_pieces | en_passant_moves
                )
                # Pushing pawns one step, if there are no pieces in the way
                single_pawn_moves = (
                    self.PAWN_MOVES_SINGLE[self.turn, i, j] & ~all_pieces
                )
                ...
                if single_pawn_moves != 0:
                    moves_to_make |= (
                        self.PAWN_MOVES_DOUBLE[self.turn, i, j] & ~all_pieces
                    )
