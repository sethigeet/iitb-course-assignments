...
def get_initial_chess_object(full_name: str):
    ...
    if not os.path.exists(
        "{}/chess/magics/{}x{}/diagonals.npz".format(minichess_path, *dims)
    ):
        print("Need to calculate magics for this board dimension variant.")
        print(
            "This only needs to be done once, but it can take a couple of minutes for large boards."
        )
        save_magic_bitboards(dims, minichess_path)

    data = np.load("{}/chess/magics/{}x{}/diagonals.npz".format(minichess_path, *dims))
    diagonal_hash_table, diagonal_magics, diag_shift = (
        data["hash_table"],
        data["magics"],
        data["shift"],
    )
    ...
    straight_hash_table, straight_magics, straight_shift = (
        data["hash_table"],
        data["magics"],
        data["shift"],
    )
    ...
    CASTLING_EMPTY_MASKS, CASTLING_ATTACK_MASKS, CASTLING_RIGHTS = castling_masks(
        dims, board_path
    )
