...
PIECE_LOOKUP = {"p": 0, "r": 3, "n": 1, "b": 2, "k": 5, "q": 4}
    ...
    for turn in [0, 1]:
        ...
        for i in range(0, dims[0]):
            if i + row_inc >= 0 and i + row_inc < dims[0]:
                for j in range(dims[1]):
                    if j != 0:
                        pawn_attacks[turn, i, j] |= B_1 << flat(
                            i + row_inc, j - 1, dims
                        )
                    if j != dims[1] - 1:
                        pawn_attacks[turn, i, j] |= B_1 << flat(
...
def castling_masks(dims, board_name):
    ...
    for p, pair in enumerate(line_pairs):
        ...
        for turn in [0, 1]:
            ...
            for j in range(dims[1]):
                if empty_mask[j] == "1":
                    empty_masks[turn, p] = set_bit(
                        empty_masks[turn, p], flat(back_rank, j, dims)
                    )
                if attack_mask[j] == "1":
                    attack_masks[turn, p] = set_bit(
                        attack_masks[turn, p], flat(back_rank, j, dims)
                    )
...
@njit
def legal_moves_to_illegal_move_mask(
    moves, proms, child_priors, all_moves, player_number
):
...
def child_U(cpuct, number_visits, child_priors, child_number_visits):
    return cpuct * np.sqrt(number_visits) * (child_priors / (1 + child_number_visits))
...
def get_best_child(
    ...
    if player_number == 0:
        return np.argmin(
            child_Q(child_win_value, child_number_visits)
            - child_U(cpuct, number_visits, child_priors, child_number_visits)
            + illegal_moves_mask * 100000
        )
    else:
        return np.argmax(
            child_Q(child_win_value, child_number_visits)
...
@njit
def prior_math(
    illegal_moves_mask, dims, child_priors, move_cap, cnoise, value_estimate, turn
):
    if turn == 0:
        value_estimate *= -1
...
def chess_move_to_uci(move, dims):
    ...
    (i, j), (dx, dy), promotion = move
    return (
        coordinate_to_square_name(i, j, dims)
        + coordinate_to_square_name(i + dx, j + dy, dims)
        + ("" if promotion == -1 else INVERSE_PIECE_LOOKUP[promotion])
    )
