...
def find_magic_bitboard(
    i, j, dims, directions_to_look, all_possible_moves, magic_shift
):
    """
    ...
    for blockers in list(itertools.product([0, 1], repeat=len(bits_to_be_occupied))):
        ...
        available_moves = (
            find_connected_components(
                ((~altered & all_moves) | origin), i, j, dims, directions_to_look
            )
            & ~origin
        )
        ...
        try:
            ...
            for i in range(dims[0]):
                for j in range(dims[1]):
                    hash_table, magic = find_magic_bitboard(
                        i, j, dims, directions_to_look, all_possible_moves, magic_shift
                    )
...
def find_magic_bitboards_for_diagonals(dims, shift):
    return find_magic_bitboards(
        dims, [(-1, 1), (1, -1), (-1, -1), (1, 1)], diagonal_line_moves(dims), shift
    )


def find_magic_bitboards_for_straights(dims, shift):
    return find_magic_bitboards(
        dims, [(-1, 0), (1, 0), (0, -1), (0, 1)], straight_line_moves(dims), shift
    )
...
def save_magic_bitboards(dims, minichess_path, shift=None):
    ...
    np.savez(
        "{}/chess/magics/{}x{}/diagonals".format(minichess_path, *dims),
        hash_table=diag_hash,
        magics=diag_magics,
        shift=diag_shift,
    )
    ...
    straight_hash, straight_magics, straight_shift = find_magic_bitboards_for_straights(
        dims, shift
    )
    np.savez(
        "{}/chess/magics/{}x{}/straights".format(minichess_path, *dims),
        hash_table=straight_hash,
        magics=straight_magics,
        shift=straight_shift,
    )
