...
class MiniChessGUI:
    def __init__(self, FENs, res, tframe):
        ...
        for ch in temp[0]:
            if ch.isalpha():
                ...
                raise ValueError("FEN parse error")
    ...
    def draw_board(self, fen):
        ...
        # Input bar
        pygame.draw.rect(
            self.screen, (50, 50, 50), (0, self.height - 80, self.width, 80)
        )
...
def main():
    ...
    parser.add_argument(
        "--fens_path",
        type=str,
        help="path to file containing FEN of the game history in each line, with final line being result of game (white=1/black=-1/draw=0)",
    )
