...
def play_matches(agent1, agent2) -> dict:
    ...
    for g in tqdm(
        range(1, NUM_GAMES + 1),
        desc=f"Playing {agent1.name} vs {agent2.name}.",
        total=NUM_GAMES,
    ):
        game_fens = []
        if g % 2 == 0:
        ...
        if save_fens:
            if res == "black" or res == "white" or res == "draw":
                if not os.path.exists(
                    f"fens/{white_agent.name}_vs_{black_agent.name}/"
                ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=0,
        type=int,
        help="Give the task number which you want to test {1,2,3}. If all tasks then 0.",
    )
    parser.add_argument(
        "--save_fens",
        action="store_true",
    ...
    if task_no == 0:
        ...
        task_1_score = (
            result_1[test_agent_1.name]["total_wins"]
            - result_1[rand.name]["total_wins"]
        )
        ...
        if result_1[test_agent_1.name]["avg_time"] > TIME_THRESHOLDS[0]:
            print(
                f"\033[38;5;208mWARNING: Your average time per turn exceeded threshold <{TIME_THRESHOLDS[0]}s \033[0;0m"
            )
        print()
        ...
        if result_2[test_agent_2.name]["avg_time"] > TIME_THRESHOLDS[1]:
            print(
                f"\033[38;5;208mWARNING: Your average time per turn exceeded threshold <{TIME_THRESHOLDS[1]}s \033[0;0m"
            )
        print()
        ...
        if result_3[test_agent_3.name]["avg_time"] > TIME_THRESHOLDS[2]:
            print(
                f"\033[38;5;208mWARNING: Your average time per turn exceeded threshold <{TIME_THRESHOLDS[2]}s \033[0;0m"
            )
        print()
        ...
        if result[test_agent.name]["avg_time"] > TIME_THRESHOLDS[task_no - 1]:
            print(
                f"\033[38;5;208mWARNING: Your average time per turn exceeded threshold <{TIME_THRESHOLDS[task_no - 1]} \033[0;0m"
            )
